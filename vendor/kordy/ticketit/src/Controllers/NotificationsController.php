...
class NotificationsController extends Controller
{
    public function newComment(Comment $comment)
    {
        ...
        $this->sendNotification($template, $data, $ticket, $notification_owner,
            trans('ticketit::lang.notify-new-comment-from').$notification_owner->first_name.trans('ticketit::lang.notify-on').$ticket->subject, 'comment');
    }
    ...
    public function ticketStatusUpdated(Ticket $ticket, Ticket $original_ticket)
    {
        ...
        if (strtotime($ticket->completed_at)) {
            $this->sendNotification($template, $data, $ticket, $notification_owner,
                $notification_owner->first_name.trans('ticketit::lang.notify-updated').$ticket->subject.trans('ticketit::lang.notify-status-to-complete'), 'status');
        } else {
            $this->sendNotification($template, $data, $ticket, $notification_owner,
                $notification_owner->first_name.trans('ticketit::lang.notify-updated').$ticket->subject.trans('ticketit::lang.notify-status-to').$ticket->status->name, 'status');
        }
    }
    ...
    public function ticketAgentUpdated(Ticket $ticket, Ticket $original_ticket)
    {
        ...
        $this->sendNotification($template, $data, $ticket, $notification_owner,
            $notification_owner->first_name.trans('ticketit::lang.notify-transferred').$ticket->subject.trans('ticketit::lang.notify-to-you'), 'agent');
    }
    ...
    public function newTicketNotifyAgent(Ticket $ticket)
    {
        ...
        $this->sendNotification($template, $data, $ticket, $notification_owner,
            $notification_owner->first_name.trans('ticketit::lang.notify-created-ticket').$ticket->subject, 'agent');
    }
    ...
    public function sendNotification($template, $data, $ticket, $notification_owner, $subject, $type)
    {
        $mail_callback = function ($m) use ($ticket, $notification_owner, $subject, $type) {
            if ($type != 'agent') {
                $m->to($ticket->user->email, $ticket->user->first_name);
                ...
                if ($ticket->agent->email != $notification_owner->email) {
                    $m->to($ticket->agent->email, $ticket->agent->first_name);
                }
            } else {
                $m->to($ticket->agent->email, $ticket->agent->first_name);
            }

            $m->replyTo($notification_owner->email, $notification_owner->first_name);
